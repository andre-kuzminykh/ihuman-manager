        ...
        *,
        ...
        try:
            ...
            system = (
                ...
                "ВАЖНО: если сообщение написал сам владелец (метка '(вы)' в "
                "контексте или текущее сообщение — от него), это ТОЖЕ может быть "
                "задачей — он мог себе напомнить или дать поручение кому-то "
                "из участников чата. Не отбрасывай только из-за того, что писал "
                "владелец."
            )
            user = f"Контекст:\n{context_text}\n\nСообщение:\n{text}"
            log.info(
        ...
        *,
        ...
        try:
            ...
            system_prompt = (
                ...
                "имена упомянутых лиц, документы, суммы, дедлайны. "
                "ВАЖНО: сообщение от владельца тоже извлекается как задача — "
                "это его поручение себе или другому участнику. Не игнорируй такие "
                "сообщения; формулируй title в нейтральном/повелительном виде "
                "(например, 'Подготовить отчёт', а не 'Я подготовлю отчёт'). "
                ...
                "ПРАВИЛА:\n"
                "1. Каждая задача — отдельный пункт. Если в сообщении несколько действий "
                "('A и B', 'A, потом B', 'A. И ещё B') — это РАЗНЫЕ задачи.\n"
                "2. Если нет ни одной задачи — tasks: [].\n"
                "3. title — короткое повелительное название до 120 символов. ЗАПРЕЩЕНО "
                "оставлять в title слова 'это', 'то', 'туда', 'ему', 'ей' без раскрытия. "
                "Если в сообщении 'отправь это' — обязательно раскрой 'это' из контекста "
                "(например, 'Отправить НДА Vest Capital'). Если контекст недостаточен — "
                "сформулируй максимально конкретно, что понятно.\n"
                "4. description — короткое, ЛАКОНИЧНОЕ описание задачи (1–2 предложения). "
                "Автора НЕ упоминай — ни «Инициатор», ни «Пользователь», ни конкретное "
                "имя/@username. Автор показывается отдельной строкой в карточке.\n"
                "   Сама задача формулируется в нейтральной форме: «Нужно ...», "
                "«Подготовить ...», «Отправить ...». Включай в description только "
                "ту фактуру, которая ДОБАВЛЯЕТ ИНФОРМАЦИЮ к title (имена/документы/"
                "суммы/сроки/кому). Если такого нет — description=null, не пиши воды "
                "вроде «В сообщении не уточнено», «требуется уточнение», «дополнительных "
                "деталей нет» и подобного.\n"
                "   Если контекст помогает уточнить 'это/то/туда' — раскрой это в "
                "title и/или description явно. Безличные местоимения без раскрытия "
                "запрещены и в title.\n"
                "5. deadline парси в Europe/Moscow. Если не указан явно — null.\n"
                "6. priority: high если есть слова 'срочно', 'asap', 'важно', 'критично', "
                "'до конца дня', 'до завтра утра'; low если 'когда будет время', "
                "'не горит', 'на досуге'; иначе medium.\n"
                "7. confidence ∈ [0,1]: насколько уверен, что это реально задача.\n"
                f"\nТекущая дата (MSK): {now_iso}."
            )
            sender_line = (
                f"\nОт кого пришло сообщение: {sender_display} "
                "(для информации; в description автора НЕ упоминай — он показывается "
                "отдельной строкой в карточке).\n"
                if sender_display
                else "\nСообщение написал сам владелец задач — автора в description не пиши.\n"
            )
            user_prompt = (
                f"Контекст последних сообщений чата:\n{context_text}\n"
                f"{sender_line}"
                f"\nТекущее сообщение для извлечения задач:\n{text}"
            )
            log.info(
                "DECOMPOSER input: model=%s sender=%r text=%r context=%r",
