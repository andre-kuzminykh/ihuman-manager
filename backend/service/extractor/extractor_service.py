        ...
        *,
        ...
        try:
            ...
            system_prompt = (
                ...
                "4. description — ОБЯЗАТЕЛЬНОЕ поле, 1–3 предложения. Должно отвечать: "
                "кто (РЕАЛЬНОЕ ИМЯ из подсказки 'От кого пришло сообщение' или "
                "из контекста; запрещены слова 'Инициатор', 'Пользователь', 'Собеседник', "
                "'Тот, кто написал' и т.п. безличные формы — только конкретное имя/@username), "
                "что именно сделать, зачем/для кого, и любые упомянутые детали "
                "(имена, документы, суммы, сроки). Бери информацию ИЗ КОНТЕКСТА ЧАТА, "
                "не только из текущего сообщения. Если контекст помогает уточнить "
                "'это/то' — раскрой это в description явно. description=null запрещён, "
                "если ты решил, что задача есть.\n"
            ...
            sender_line = (
                ...
                "ИСПОЛЬЗУЙ ЭТО ИМЯ в description вместо 'Инициатор', 'Пользователь', "
                "'Собеседник'. Если есть полное имя — пиши полное имя. Если только "
                "@username — используй его.\n"
                if sender_display
                else ""
            )
            user_prompt = (
